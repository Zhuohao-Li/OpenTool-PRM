...
import re
import numpy as np
from verl.utils.reward_score.pc_reward import compute_score_llm_judge, write_tag_rewards, filter_llm_judge_score
...
class RewardManager():
    ...
    def __init__(self, tokenizer, num_examine, format_score=0., config=None) -> None:
        self.tokenizer = tokenizer
        self.num_examine = num_examine  # the number of batches of decoded responses to print to the console
        self.format_score = format_score
        self.include_llm_judge_score = config.include_llm_judge_score
        self.llm_judge_model_id = config.llm_judge_model_id
        
        # Parallel processing configuration
        self.max_workers = getattr(config, 'llm_judge_max_workers', 8)  # Default to 8 workers
        self._lock = threading.Lock()  # Thread lock for shared resources

    def _process_single_llm_judge(self, rollout_result):
        ...
        reward_tensor = rollout_result.get('reward_tensor', None)
        
        print('Begin to judge process through PRM')
        print(f"Response to be scored: {response_str}")
        ...
        try:
            ...
            is_score_valid = filter_llm_judge_score(score_llm_result, response_str)
            ...
            if is_score_valid:
                ...
                return 1, 1  # total_judge_count, valid_judge_count
            else:
                return 1, 0  # total_judge_count, valid_judge_count
    ...
    def __call__(self, data: DataProto):
        ...
        for i in range(len(data)):
            ...
            # Store result for potential LLM judge
            rollout_result = {
                'index': i,
        ...
        if self.include_llm_judge_score:
            ...
            with ThreadPoolExecutor(max_workers=max_workers) as executor:
                future_to_rollout = {
                    executor.submit(self._process_single_llm_judge, rollout): rollout 
